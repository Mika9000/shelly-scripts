...
Synopsis: Shelly script to control an electric hot water heater in a solar panel equipped household.

**But**, in addition to merely switching the heating on when the sun shines, it does the following:

1. In the morning, the heater script wakes up, heating the water when the power generated by the solar panel system exceeds the power requirement of the heater. When the heater&apos;s thermostat opens or the allowed heating time has been used up, the heating is marked as finished and turns off for the rest of the day.
...
The software consists of two Shelly scripts: **Reader** and **Control**, and they run on a switch that controls the water heater&apos;s contactor.
...
### Description
This is the timer and data query component. It times and synchronises the operation and calls API endpoints and the control script.

The script fetches the amount of energy available, water heater power consumption, low electricity price status, and marks scheduled events. Once it has collected the data, it calls the control script and passes it the values.

Copy and paste the javascript code into the first script slot (id 1) of the Shelly and name it **Reader**. Adjust script&apos;s CONFIG parameters to match the environment.
...
Energy meter call that returns the water heater&apos;s power consumption. Url must include full path including the channel.
...
Call returns response code 200 OK if the current hour belongs to the cheap hours, and 400 Bad Response if it doesn&apos;t. Use of this endpoint is optional and can be safely removed from the list if not needed.
...
Url of the control script that this script calls. It must include the script id and the endpoint name.

**dayBegins**

Time when the control script is awakened and prepared to expect full heating power from the solar panel array (all clock times are 24hrs).
...
Tells control script that the reading is invalid. Value must be greater than any reading ever can be, and be the same in the Reader and Control script.

**schedule**

Defines how often the reader runs. The parameter of interest is **timespec**, which specifies a cron job like interval.

## Control script

### Description

The control script is the actual water heater controller. The reader script calls it at regular intervals with the readings it has collected and causes it to execute one control cycle.

Copy and paste the javascript code into the second script slot (id 2) of the Shelly and name it **Control**. Adjust script&apos;s CONFIG to match the environment.
...
Marker that the received reading is invalid. Value must be greater than any reading ever can be, and be the same in the Reader and Control script.
...
Reader script installs a schedule, which in this example runs every 5 minutes. As long as the heating is unfinished, the script goes to collect new readings by the schedule and calls the control script and passes it the values.

Control script installs an endpoint that listens for commands. When it receives a call it picks up the values the reader passes and runs a single control cycle. At the end of each cycle it sends a report to home automation endpoint (see parameter **homeEndpoint**) to tell how the heating is progressing (optional).

When the control script detects that the heating time is full or the heater thermostat has opened, it marks the heating as complete by setting a value 1 (heating completed) to the key defined in **kvsControlKey** parameter. Reader script monitors this key, and when it sees this value value it stops calling endpoints and the control script until the next morning. In the morning, the reader wakes up based on the time set in **dayBegins** setting, calls the the control script to reset all values, and a new heating cycle begins.
...
If heating was unfinished, it will start over because the control script lost track of the elapsed heating time.
